...
export const ProductCard = ({ product }: ProductProps) => {
  ...
  // 1. Resolução de Dados (Fallback robusto)
  ...
  const images = product.images && product.images.length > 0 
    ...
    : [product.image_url || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=400'];

  // 2. Lógica de Desconto Inteligente
  ...
  // 3. Status de Sincronização (Data relativa)
  const getSyncStatus = () => {
    const dateStr = product.updated_at || product.last_sync;
    if (!dateStr) return 'Preço verificado';
    const dateObj = new Date(dateStr);
    return isValid(dateObj) 
      ? `Atualizado ${formatDistanceToNow(dateObj, { addSuffix: true, locale: ptBR })}`
      : 'Preço verificado';
  };

  // 4. Identificação do Marketplace
  ...
  const handleFavorite = async (e: React.MouseEvent) => {
    ...
    // Otimistic update (feedback instantâneo visual)
    const previousFavorites = queryClient.getQueryData(['favorites', user.id]);
    
    try {
      if (isFavorited) {
        await supabase.from('favorites').delete().eq('product_id', product.id).eq('user_id', user.id);
        toast.success('Removido dos favoritos');
      } else {
        await supabase.from('favorites').insert({ product_id: product.id, user_id: user.id });
  ...
  const purchaseLink = product.checkout_url || product.affiliate_link || product.source_url || undefined;
    ...
    >
      <Card className="h-full flex flex-col group relative overflow-hidden border-zinc-800 bg-zinc-900 rounded-[26px] shadow-lg hover:shadow-[#a3e635]/20 hover:border-[#a3e635]/50 transition-all duration-300 focus-within:ring-2 focus-within:ring-[#a3e635]/40">
        ...
        <div className="relative aspect-square overflow-hidden bg-white p-6">
          ...
          {/* Badge de Desconto (Canto Inferior) */}
          <AnimatePresence>
            {discount && (
              <motion.div 
        ...
        <div className="flex flex-col flex-1 p-5">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className={`text-[10px] font-bold uppercase tracking-widest flex items-center gap-1 ${marketInfo.color}`}>
