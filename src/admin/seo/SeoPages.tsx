  ...
  const { data, isLoading, isFetching, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: rows, error, count } = await supabase
        ...
        .select("id,slug,cluster_level,cluster_keyword,index_status,traffic_estimate,updated_at", { count: "exact" })
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Paged listing of generated SEO pages with cluster and index metadata.
        </p>
      </div>
      <Card>
