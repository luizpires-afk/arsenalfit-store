...
begin
  ...
  v_affiliate_bound := v_affiliate;
  if v_affiliate_bound is not null
     and public.is_mercadolivre_sec_link(v_affiliate_bound)
     and v_canonical_ml_item_id is not null then
    if strpos(v_affiliate_bound, '?') > 0 then
      v_affiliate_bound := v_affiliate_bound || '&item_id=' || v_canonical_ml_item_id;
    else
      v_affiliate_bound := v_affiliate_bound || '?item_id=' || v_canonical_ml_item_id;
    end if;
  end if;
  ...
  if lower(coalesce(v_product.auto_disabled_reason, '')) = 'blocked' then
    ...
    if v_is_active and public.is_mercadolivre_sec_link(v_affiliate) then
      v_target := coalesce(v_affiliate_bound, v_affiliate);
      v_target_source := 'affiliate';
      v_reason := case
        when v_canonical_ml_item_id is not null then 'affiliate_bound_to_canonical_item'
        else 'affiliate_validated'
      end;
  ...
  v_destination_ml_item_id := coalesce(
    ...
    case
      when v_target is not null and v_target_source in ('canonical_source', 'source', 'affiliate') then v_canonical_ml_item_id
      else null
    end
  );
