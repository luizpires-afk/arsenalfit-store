-- Gate de coerencia para fila de validacao SEC em standby:
-- so exporta produtos com vinculo de item consistente e sem mismatch/suspeita aberta.
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      'limit', v_limit,
      'coherence_gate', true,
      'excluded_health_status', array['PRICE_MISMATCH', 'SUSPECT_PRICE', 'API_MISSING']
    )
  )
  ...
  with pending_raw as (
    ...
    where lower(coalesce(p.marketplace, '')) like 'mercado%'
      ...
      and coalesce(p.data_health_status, 'HEALTHY') not in ('PRICE_MISMATCH', 'SUSPECT_PRICE', 'API_MISSING')
