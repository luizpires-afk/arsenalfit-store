...
begin
  ...
  for rec in
    ...
    last_api_base as (
      select
        ...
        max(e.created_at) filter (
          where e.final_price_source = 'API_BASE'
    ...
    select
      ...
      case
        ...
        when public.extract_ml_item_id_from_url(p.affiliate_link) is null then 'affiliate_ml_item_unbound'
        when public.extract_ml_item_id_from_url(p.affiliate_link) is distinct from public.normalize_ml_external_id(p.ml_item_id)
        ...
        when l.last_api_base_at is null then 'no_recent_api_base_confirmation'
        else null
      end as fail_reason
  ...
  return jsonb_build_object(
    ...
    'requires_affiliate_ml_item_binding', true
  );
end;
$$;
