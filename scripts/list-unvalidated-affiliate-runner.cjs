...
const main = async () => {
  ...
  const rpcRows = await client.rpc("list_unvalidated_affiliate_products", { p_limit: limit });
  const rows = sortRows(
    (Array.isArray(rpcRows) ? rpcRows : []).map((row) => ({
      id: row.id,
      name: row.name,
      status: row.status,
      is_active: Boolean(row.is_active),
      affiliate_verified: Boolean(row.affiliate_verified),
      reason_code: row.reason_code,
      deactivation_reason: row.deactivation_reason || null,
      auto_disabled_reason: row.auto_disabled_reason || null,
  ...
  if (!rows.length && allMercado.length) {
  ...
  const payload = {
    ...
    error_summary: countBy(rows, (row) => String(row.reason_code || "sem_motivo")),
    rows,
  };
  ...
  fs.writeFileSync(
    txtPath,
    `${rows.map((row) => `${row.id}\t${row.reason_code}\t${row.status}`).join("\n")}${rows.length ? "\n" : ""}`,
    "utf8",
  );

  if (asJson) {
    console.log(JSON.stringify({ ...payload, outputs: { json: jsonPath, csv: csvPath, txt: txtPath } }, null, 2));
    return;
  }
    ...
    JSON.stringify(
      {
        ...
        outputs: { json: jsonPath, csv: csvPath, txt: txtPath },
      },
      null,
      2,
    ),
